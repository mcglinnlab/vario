...
##3.2##
vario<-function(x,coord,grain=1,breaks=NA,hmax=NA,round.up=FALSE,pos.neg=FALSE,binary=TRUE,snap=NA,median=FALSE,
 ...
 ##Arguments:
 ##'x' an object of class 'sim' or may be a sitexsp matrix, a vector of values, if x is the later then missing samples should be coded as -999
 ...
 ##'pos.neg' if TRUE the positive and negative parts of the covariance matrix should be output
 ##'binary' if TRUE and x is class sim then abundances are converted to binary 0 1 values
 ##'snap' indicates which generation from an object class 'sim' to draw from
 ##'median' indicates if in addition to the mean the medians of the distance matrices are calculated
 ####geoR arguments####
 ...
 if(class(x)=="sim"){
  ...
  vobject$parms <- data.frame(grain,hmax,S=ncol(resp),N=nrow(resp),pos.neg,median,
                             niche.wid.rel=x$p$s.rel,disp.wid.rel=x$p$u.rel, direction,
                             tolerance,unit.angle,distance.metric)
 } 
 else{
  resp <- as.matrix(x)
  vobject$parms <- data.frame(grain,hmax,S=ncol(resp),N=nrow(resp),pos.neg,median,
                            direction,tolerance,unit.angle,distance.metric)
 }
 H[H > hmax] <- NA
 H <- as.vector(H)
 ####geoR code####
 if (direction != "omnidirectional") {
  ##note that the changes here where 'u' has been changed for as.vector(Dist)
  ##and coords changed to coord
 ...
 vobject$vario <- data.frame(H = names(table(H)), Dist = Dist, n = as.numeric(table(H)))
 ##below 'exp.gamma' is the expected variogram if 'resp' is a sitexsp pres/abse matrix 
 ##the expectation is based upon the assumption of zero spxsp covariances
 if(!is.null(distance.metric))
  exp.split <- split(vegdist(resp,method=distance.metric),H) #turnover metric computed
 else 
  exp.split <- split(dist(resp)^2*.5,H) #euclidean distance computed
 ...
 if(ncol(resp)>1){#i.e. it is a sp x site matrix and not simply a vector
  ##if 'resp' is a sitexsp pres/abse matrix the following computes site species richness
  rich<-apply(resp,1,sum) 
  ##see equation 7 in Wagner, H. 2003. Spatial covariance in plant communities... Ecology 84:1045-1057.
  ##to see that observed multivariate variogram can be computed from the species richness vector
  obs.gamma <- sapply(split(dist(rich)^2*.5,H),mean,na.rm=TRUE) ##observed variogram
  if(pos.neg){
   ...
   if(median){
    ...
    vobject$vario <-cbind(vobject$vario, obs.var = obs.gamma, ratio = obs.gamma/exp.gamma, pos = pos, neg = neg, exp.med = exp.med, pos.med = pos.med, neg.med = neg.med)
   }
   else
    vobject$vario <-cbind(vobject$vario, obs.var = obs.gamma, ratio = obs.gamma/exp.gamma, pos = pos, neg = neg)
   ##Note: obs.var = exp.var + pos.var + neg.var
  }
  else{
   if(median){
    obs.gamma.med <- sapply(split(dist(rich)^2*.5,H),median,na.rm=TRUE) ##observed variogram
    vobject$vario <-cbind(vobject$vario, obs.var = obs.gamma, ratio = obs.gamma/exp.gamma, exp.med = exp.med, obs.med = obs.gamma.med)
   }
   else
    vobject$vario <-cbind(vobject$vario, obs.var = obs.gamma, ratio = obs.gamma/exp.gamma)
 }} 
 vobject$p <- apply(resp,2,sum,na.rm=TRUE)/nrow(resp)
 vobject$perm <- FALSE ##this flag indicates to the function 'v.graph' if this is the result of a randomization or not
 vobject
}
...
v.graph<-function(vobject,optim=NA,exp.only=FALSE,flip.neg=FALSE,ylim=NULL,xlab=NULL,ylab=NULL,cls=NULL){
 ...
 if(vobject$perm){ ##if vobject is result of permutations
  ...
  if(vobject$parms$pos.neg){
   ...
   plot(d,v[,1,1],pch=19,type='o',col='green3',ylab=ylab,xlab=xlab,ylim=ylim)
   lines(d,v[,2,1],col=1,lwd=2,type='o',pch=19)
  }

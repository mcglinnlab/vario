/* 
AUTHOR: Dan McGlinn
CONTACT: dmcglinn@email.unc.edu
DATE: 10/22/2010
PURPOSE: The purpose of this alogrithim is to implement the random patterns algo
described in Roxburgh and Chesson (1998).  Towards this end this algo accepts an
initial observed arrangement of occurances and an initially nullfied arrangedment
of occurances.  The output is a null arrangement of occurances that approximates
the observed arrangement in terms of their first order spatial statistics. These
tatistics include: E = # number of neighbors in cardinal directions, C = # number
of neighbors in intermediate directions (i.e., diagonal directions), S = # number 
of times all sampled neighbors are occupied, and  O = # of times all the sampled
neighbors are unoccupied. There are important differences in the implementation 
of this algo and that described in  Roxburgh & Chesson.  The primary difference
is that here we do not have special counting rules of neighbors if you are along
the edges of the landscape. Instead we have artifically extended the landscape 
one pixel in each direction prior to inputing it to this function. This allows
the algo to search outside of the observed spatial domain by one pixel. My 
primary reason for doing this is that this drops the reqirement of treating 
edges of the observed grid differently from internal cells.  Additionally 
becuase this algo will be given sets of cells in which there are no observations
it would be quite complex if we started counting the spatial stats specially
for each place we have no sampled data (i.e., edge of a grid or empty spot). 
Unsampled grid cells have an abundance less than -998, the algorithim takes this 
into consideration in the sub function spatstat.

REFERENCE:
Roxburgh, S.H. Chesson, P.A. New Method for Detecting Species Associations with
  Spatially Autocorrelated Data. Ecology. 79: 2180-2192.
ARGUMENT DEFINITIONS:
psp:     a vector of real numbers that specifies presences/abundances in a 2-D
         spatial sampling grid, that has been modified by adding sites around the
         grid for the purposes of this algo, -998 indicates an unsampled pixel
...
gtrials: a counter for the number of times an attempted swap was a 'good' swap (i.e., it lowered phi)
